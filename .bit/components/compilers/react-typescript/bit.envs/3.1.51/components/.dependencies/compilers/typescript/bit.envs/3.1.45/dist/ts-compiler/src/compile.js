...
function _compile(context, cc, preset) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield runCompiler(() => runNodeScriptInDir(context.directory, pathToCompiler, compilerArguments), preset);
...
function findMainFile(context, dists) {
    const compDistRoot = path_1.default.resolve(context.directory, tsconfig_1.FIXED_OUT_DIR);
    const sourceFileName = getNameOfFile(context.main, context.main.substring(context.main.indexOf('.')));
    const pathPrefix = `${compDistRoot}${compDistRoot.endsWith(path_1.default.sep) ? '' : path_1.default.sep}`;
...
function createConfigFile(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const content = context.cc.dynamicConfig[getNameOfFile(configFileName, '.')];
...
function collectDistFiles(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const compDistRoot = path_1.default.resolve(capsuleDir, tsconfig_1.FIXED_OUT_DIR);
        const files = yield recursive_readdir_1.default(compDistRoot);
        const readFiles = yield Promise.all(files.map((file) => {
            return fs.readFile(file);
        }));
        return files.map((file, index) => {
            const relativePath = path_1.default.relative(path_1.default.join(capsuleDir, tsconfig_1.FIXED_OUT_DIR), file);
...
function collectNonDistFiles(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const ignoreFunction = function (file, _stats) {
            ...
            const defaultIgnore = [`node_modules${path_1.default.sep}`, tsconfig_1.FIXED_OUT_DIR, '.dependencies'].concat(dynamicConfig.compiledFileTypes);
            return defaultIgnore.concat(copyPolicy.ignorePatterns).reduce(function (prev, curr) {
                return prev || !!~file.indexOf(curr);
            }, false);
        };
        const fileList = yield recursive_readdir_1.default(capsuleDir, [ignoreFunction]);
        const readFiles = yield Promise.all(fileList.map((file) => {
            return fs.readFile(file);
        }));
...
function getConfigFilePath(context) {
    return path_1.default.join(context.directory, context.cc.dynamicConfig.configFileName);
}
...
function getCapsuleName(ctx) {
    ...
    const componentInProjectId = md5_1.default(`${ctx.context.rootDistDir}${name}${version}`);
    const targetDir = path_1.default.join(os_1.default.tmpdir(), 'bit', componentInProjectId);
    return targetDir;
}
